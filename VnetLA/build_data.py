...
# --------------------------------------------------------------------------------
def get_idx(root, list_path,train=True, label_num=5):
...
class BuildDataLoader:
    def __init__(self, dataset, num_labels,base_data_path):
        self.dataset = dataset
        if dataset == 'la':
            self.im_size = [112, 112, 80]
            self.crop_size = [112, 112, 80]
            self.num_segments = 2
            self.scale_size = (0.5, 1.5)
            self.batch_size = 2
            self.data_path =base_data_path+"/data/"
            self.data_list_path=base_data_path+"/datalist/"
            self.train_l_idx, self.train_u_idx = get_idx(self.data_path, self.data_list_path,train=True, label_num=num_labels)
            self.val_idx = get_idx(self.data_path, self.data_list_path,train=False)
