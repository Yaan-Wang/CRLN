...
from utilsm.feature_memory import *
...
def calculate_current_acc(preds, label_batch,threshold):
    mask = (torch.softmax(preds, dim=1).max(1)[0] > 0)
...
def bootstrap_evaluation(model, op_module, test_loader, num_samples=100, save_path=None, load_path=None, metrics_save_file=None):
    metrics_list = []
    ...
    if load_path:
        ...
        for i in tqdm(range(num_samples), desc='Bootstrapping Iterations'):
            ...
            if save_path:
                save_sample_names(sampled_name_list, f"{save_path}_sample_{i}.txt")
...
results_log = open(config["log_dir"], 'a')
...
mean_metrics, std_metrics = bootstrap_evaluation(
    model, op_module, test_loader, num_samples=100,
    save_path=config["save_path"], load_path=config["load_path"],metrics_save_file=config["log_dir"])

# Log results
print(f'Bootstrapping Average metrics: {mean_metrics} ± {std_metrics}')
with open(config["log_dir"], 'a') as results_log:
