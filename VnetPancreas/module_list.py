...
def compute_unsupervised_loss(predict, target, logits, strong_threshold):
    ...
    loss = F.nll_loss(torch.log_softmax(predict, dim=1),
                                   target, reduction="none", ignore_index=-1)
...
# Define cps loss
# --------------------------------------------------------------------------------
def compute_cps_loss(rep, label, mask, prob, cls=None,strong_threshold=1.0, temp=0.5, num_queries=256, num_negatives=256,con_xi=0.6):
    batch_size, num_feat, im_w_, im_h,im_c = rep.shape
    ...
    if len(seg_num_list) <= 1:  
        ...
        for i in range(valid_seg):
            ...
            with torch.no_grad():
                ...
                if cls is not None:
                   class_pre=(cls[0,i,:,:]).unsqueeze(0).permute(0,2,1)
                   class_pre=torch.mean(class_pre,dim=1,keepdim=True).repeat(num_queries, 1, 1)
                   positive = seg_proto[i].unsqueeze(0).unsqueeze(0).repeat(num_queries, 1, 1)
                   positive_feat=(positive+con_xi*class_pre)/(1+con_xi)
                else:
            ...
            reco_loss = reco_loss + F.cross_entropy(seg_logits / temp, torch.zeros(num_queries).long().to(device), ignore_index=-1)
...
def label_onehot(inputs, num_segments):
    batch_size, im_h, im_w,in_c = inputs.shape
    ...
    outputs = torch.zeros([batch_size, num_segments, im_h, im_w,in_c]).to(inputs.device)
...
def denormalise(x, imagenet=True):
    ...
    if imagenet:
        x = Normalise(x)
        # x = Normalise(x, mean=[-0.485, -0.456, -0.406], std=[1., 1., 1.])
        return x
    else:
        return (x + 1) / 2
...
def tensor_to_pil(im, label, prob):
    im = denormalise(im).squeeze(0)
    im=im.cpu().numpy()
...
def generate_unsup_data(data, target, logits, mode='cutout'):
    # print(data.shape)
    batch_size, _, im_h, im_w,in_c = data.shape

    mix_data = data.clone()
    mix_target = target.clone()
    mix_logits = logits.clone()
    u_rand_index = torch.randperm(data.size()[0])[:data.size()[0]].cuda()

    u_bbx1, u_bby1, u_bbx2, u_bby2 = rand_bbox(data.size(), lam=np.random.beta(4, 4))
    for i in range(0,batch_size):
        if i < mix_data.shape[0]:
